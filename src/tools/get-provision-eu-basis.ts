/**
 * get_provision_eu_basis — Get EU legal basis for a specific Japanese provision.
 */
...
export async function getProvisionEUBasis(
  ...
  const articleNum = parseArticleNumber(input.provision_ref);
  const provRefSearch = `art-${articleNum}`;

  const provision = db.prepare(
    'SELECT id, content FROM legal_provisions WHERE document_id = ? AND (provision_ref = ? OR section = ? OR provision_ref = ?)'
  ).get(resolvedId, provRefSearch, articleNum, input.provision_ref) as { id: number; content: string } | undefined;
  ...
  interface Row {
    id: string; type: 'directive' | 'regulation' | 'decision'; title: string | null;
