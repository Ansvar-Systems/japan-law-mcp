/**
 * search_eu_implementations — Search EU directives/regulations that relate to Japanese statutes.
 */
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      COUNT(DISTINCT er.document_id) AS jp_statute_count,
  ...
  if (input.has_jp_implementation !== undefined) {
    sql += input.has_jp_implementation ? ` HAVING jp_statute_count > 0` : ` HAVING jp_statute_count = 0`;
  }
  ...
  interface Row {
    ...
    jp_statute_count: number; primary_implementations: string | null;
  }
  ...
  return {
    results: {
      results: rows.map(r => ({
        ...
        jp_statute_count: r.jp_statute_count,
